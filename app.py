...
def run_ocr_on_image(img_bytes, filename="file.jpg"):
    """ 輔助函數：將圖片送去 OCR 進行強效無格仔還原 """
        ...
        with st.spinner("🤖 正在提取並轉化為無格仔純文字..."):
            try:
                ...
                if file_name.endswith('.pdf') and HAS_PDF:
                    ...
                    # 如果直接讀出來係空白（代表係掃描 PDF），轉用圖片 OCR 處理
                    if not raw_text.strip() and HAS_PDF2IMAGE:
                        images = convert_from_bytes(file_bytes)
                        ocr_results = []
                        for img in images:
                            img_byte_arr = io.BytesIO()
                            img.save(img_byte_arr, format='JPEG')
                            ocr_results.append(run_ocr_on_image(img_byte_arr.getvalue(), "page.jpg"))
                        raw_text = "\n".join(ocr_results)
                else:
                    image = Image.open(io.BytesIO(file_bytes))
                    img_byte_arr = io.BytesIO()
                    image.save(img_byte_arr, format=image.format if image.format else 'JPEG')
                    raw_text = run_ocr_on_image(img_byte_arr.getvalue(), uploaded_file.name)
