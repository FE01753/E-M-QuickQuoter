...
st.write("上載 PDF 或相片，直接還原為專業報價單 Markdown 表格格式！")
            ...
            try:
                if file_name.endswith('.pdf') and HAS_PDF:
                    ...
                    if res.get('ParsedResults') and res['ParsedResults'][0].get('TextOverlay'):
                        ...
                        # 建立 Markdown 表格開頭
                        markdown_table = "| 項目 Item | 內容 Descriptions | 數量 Qty | 單價 Unit Prince | 金額 Price |\n"
                        markdown_table += "| :--- | :--- | :---: | :---: | :---: |\n"
                        ...
                        for row in rows:
                            ...
                            # 簡單模擬分欄（根據 X 坐標或直接組合）
                            row_texts = [item['text'] for item in row['items']]
                            full_line = " ".join(row_texts)
                            
                            # 過濾掉表頭重覆字眼
                            if any(w in full_line.lower() for w in ["項目 item", "descriptions", "數量 qty", "單價unit prince", "金額price"]):
                                continue
